...
def init():
    ...
    try:
        secret_1 = Path('secret_1').read_bytes()
        secret_2 = Path('secret_2').read_bytes()
        flag_token = Path('flag_token').read_bytes()
...
def rebuild():
    ...
    Path('secret_1').write_bytes(secret_1)
    Path('secret_2').write_bytes(secret_2)
    Path('flag_token').write_bytes(flag_token[0]+flag_token[1])
